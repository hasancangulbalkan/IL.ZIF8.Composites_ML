...
features = tpot_data.drop('CH4 Uptake (mol/kg) - 1 bar', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CH4 Uptake (mol/kg) - 1 bar'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.000526311517797056
exported_pipeline = make_pipeline(
    RobustScaler(),
    StackingEstimator(estimator=RidgeCV()),
    GradientBoostingRegressor(alpha=0.75, learning_rate=0.1, loss="huber", max_depth=8, max_features=0.35000000000000003, min_samples_leaf=17, min_samples_split=9, n_estimators=100, subsample=0.8500000000000001)
)
...
plt.xlim(0, 0.55) # Set the x-axis limits 
plt.ylim(0, 0.55) # Set the y-axis limits 
...
ax.text(0.02, 0.97, '(b) 1 bar', transform=ax.transAxes, ha='left', va='top', fontsize=18)
text = '$\mathrm{R}^2$:0.952\nRMSE:0.024\nMAE:0.018\nSRCC:%.3f' % srcc_test
ax.text(0.97, 0.03, text, transform=ax.transAxes, ha='right', va='bottom', multialignment='left', fontsize=16, color='#f03592')
text = '$\mathrm{R}^2$:0.984\nRMSE:0.012\nMAE:0.008\nSRCC:%.3f' % srcc_train
