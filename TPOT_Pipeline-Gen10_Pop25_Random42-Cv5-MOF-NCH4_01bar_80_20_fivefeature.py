...
# Average CV score on the training set was: -8.155879760063333e-06
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    GradientBoostingRegressor(alpha=0.9, learning_rate=0.5, loss="huber", max_depth=3, max_features=0.6000000000000001, min_samples_leaf=10, min_samples_split=17, n_estimators=100, subsample=0.7000000000000001)
)
...
ax.text(0.02, 0.97, '(a) 0.1 bar', transform=ax.transAxes, ha='left', va='top', fontsize=18)
text = '$\mathrm{R}^2$:0.899\nRMSE:0.0036\nMAE:0.0027\nSRCC:%.3f' % srcc_test
ax.text(0.97, 0.03, text, transform=ax.transAxes, ha='right', va='bottom', multialignment='left', fontsize=16, color='#f03592')
text = '$\mathrm{R}^2$:0.982\nRMSE:0.0013\nMAE:0.0009\nSRCC:%.3f' % srcc_train
